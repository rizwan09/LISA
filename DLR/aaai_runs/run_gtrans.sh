...
PYTHONPATH=${SRC_DIR} CUDA_VISIBLE_DEVICES=$RGPU python2 ${SRC_DIR}/examples/GraphParser.py \
    --mode FastLSTM \
    --no_CoRNN \
    --hidden_size 300 \
    --num_layers 6 \
...
do
        PYTHONPATH=${SRC_DIR} CUDA_VISIBLE_DEVICES=$RGPU python2 ${SRC_DIR}/examples/analyze.py --parser biaffine --ordered --gpu\
                    --punctuation 'PUNCT' 'SYM' '.' --out_filename analyzer.${cur_lang}.out --model_name 'graph_trans.pt' \
