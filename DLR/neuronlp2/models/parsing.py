                ...
                if position_dim > 0:
                    ...
                    if not train_position:
                        ...
                        position_enc = np.array([[pos / np.power(10000, 2 * (j // 2) / pos_emb_size) for j in
                                                  range(pos_emb_size)] if pos != 0 else np.zeros(pos_emb_size) for pos
                                                 in
...
class StackPtrNet(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, kernel_size,
                 ...
                 dec_dim_feature=10, dec_drop_f_embed=0., use_self_attn=False,
                 enc_clip_dist=0, enc_use_neg_dist=False, partitioned=False,
        ...
        self.multi_head_attn = multi_head_attn
        assert not (self.use_con_rnn and self.multi_head_attn), \
            "both `use_con_rnn` and `multi_head_attn` flags are set to True"
        #
        ...
        if self.partitioned:
            ...
            assert self.position_dim > 0, "position dim must be > 0 when partitioned flag is on"
            assert self.input_concat_position
        ...
        input_sizes = {'word_dim': 0, 'char_dim': 0, 'pos_dim': 0}
        if use_word_emb:
            input_sizes['word_dim'] = word_dim
        if char:
            input_sizes['char_dim'] = num_filters
        if pos:
            input_sizes['pos_dim'] = pos_dim

        if input_concat_embeds:
            dim_enc = sum(input_sizes.values())
        else:
            # all the embeddings will be added
            assert len(set(input_sizes.values())) == 1, "embedding size mismatch"
            dim_enc = input_sizes.values()[0]
        ...
        if self.use_con_rnn:
            self.encoder = RNN_ENCODER(dim_enc, hidden_size, num_layers=encoder_layers, batch_first=True,
                                       bidirectional=True, dropout=p_rnn)
            enc_output_dim = 2 * hidden_size
            enc_hidden_dim = 2 * hidden_size
            if self.use_self_attn:
                self.linear_query = nn.Linear(enc_output_dim, enc_output_dim)
                self.linear_key = nn.Linear(enc_output_dim, enc_output_dim)
                enc_output_dim = enc_output_dim * 2

        else:
            if self.multi_head_attn:
                d_model = position_dim + dim_enc if input_concat_position else dim_enc
                if not input_concat_position:
                    assert position_dim == dim_enc, "positional embedding size mismatch"
                ...
                self.transformer = TransformerEncoder(self.encoder_layers,
                                                      ...
                                                      d_content=dim_enc)
                enc_output_dim = d_model
                enc_hidden_dim = enc_output_dim

            else:
                self.self_attn = nn.Linear(dim_enc, dim_enc)
                enc_output_dim = dim_enc
                enc_hidden_dim = enc_output_dim

            if self.pool_type == 'weight':
                self.weight_pool = nn.Linear(enc_output_dim, 1)
        ...
        self.hx_dense = nn.Linear(enc_hidden_dim, hidden_size)
    ...
    def _get_encoder_output(self, input_word, input_char, input_pos, mask_e=None, length_e=None, hx=None):
        ...
        if self.use_con_rnn:
            # output from rnn [batch, length, hidden_size]
            output, hn = self.encoder(src_encoding[0], mask_e, hx=hx)
            if self.use_self_attn:
                ...
                I = Variable(I).cuda() if self.use_gpu else Variable(I)
            ...
            if self.multi_head_attn:
                ...
                hw = self.self_attn(src_encoding)  # B x S x h
                attn_weights = torch.bmm(src_encoding, hw.transpose(1, 2))  # B x S x S
                attn_weights = F.softmax(attn_weights, dim=-1)  # B x S x S
            ...
            if self.pool_type == 'mean':
                ...
                att_weights = self.weight_pool(src_encoding).squeeze(2)  # B x S
