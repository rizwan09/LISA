...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, d_k, d_v,
                 dropout=0.1, clip_dist=0, use_neg_dist=False,
                 d_content=None):
        ...
        if self.partitioned:
            ...
            self.linear_keys1 = nn.Linear(self.d_content, head_count * (d_k // 2))
            self.linear_query1 = nn.Linear(self.d_content, head_count * (d_k // 2))
            self.linear_values1 = nn.Linear(self.d_content, head_count * (d_v // 2))

            self.linear_keys2 = nn.Linear(self.d_position, head_count * (d_k // 2))
            self.linear_query2 = nn.Linear(self.d_position, head_count * (d_k // 2))
            self.linear_values2 = nn.Linear(self.d_position, head_count * (d_v // 2))
        ...
        if self.partitioned:
            self.final_linear1 = nn.Linear(self.head_count * (d_v // 2), self.d_content, bias=False)
            self.final_linear2 = nn.Linear(self.head_count * (d_v // 2), self.d_position, bias=False)
        else:
    ...
    def forward(self, key, value, query, mask=None):
        ...
        if self.partitioned:
            key_up1 = shape(self.linear_keys1(key[:, :, :self.d_content]), self.d_k // 2)
            key_up2 = shape(self.linear_keys2(key[:, :, :self.d_position]), self.d_k // 2)
            key_up = torch.cat([key_up1, key_up2], dim=-1)  # bsz x nhead x key_len x d_k

            value_up1 = shape(self.linear_values1(value[:, :, :self.d_content]), self.d_v // 2)
            value_up2 = shape(self.linear_values2(value[:, :, :self.d_position]), self.d_v // 2)
            value_up = torch.cat([value_up1, value_up2], dim=-1)  # bsz x nhead x key_len x d_v

            query_up1 = shape(self.linear_query1(query[:, :, :self.d_content]), self.d_k // 2)
            query_up2 = shape(self.linear_query2(query[:, :, :self.d_position]), self.d_k // 2)
        ...
        if self.partitioned:
            ...
            context_l = context[:, :, :, :(self.d_v // 2)].contiguous()
            context_l = context_l.view(batch_size, value_len, -1)
            context_n = context[:, :, :, (self.d_v // 2):].contiguous()
