...
arg_parser.add_argument('--test_parse_files',
                        help='Comma-separated list of test data files')
arg_parser.add_argument('--dev_parse_files',
                        help='Comma-separated list of development data files')
...
def eval_fn(input_op, sess):
  ...
  while True:
    ...
    try:
      ...
      input_all_p = sess.run(input_op)
      pdb.set_trace()
      input_np = input_all_p['features']
      predictor_input = {'features': input_np, 'parse_tree':input_all_p['parse_tree']}
